      ...
      <div style={{display:"flex",alignItems:"center",gap:8,justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{fontSize:11,color:T.txM,fontWeight:500,letterSpacing:0.3,display:"flex",alignItems:"center",gap:4}}>
            {label}{tooltip&&<span style={{fontSize:11,color:T.txD,marginLeft:2}}>ⓘ</span>}
          </span>
        </div>
        {badge&&<span style={{fontSize:11,fontWeight:700,padding:"2px 7px",borderRadius:999,background:colorBg,color,letterSpacing:0.5}}>{badge}</span>}
      </div>
      <div style={{fontSize:22,fontWeight:700,color:T.tx,letterSpacing:-0.3,fontFamily:"'JetBrains Mono','SF Mono',Consolas,monospace"}}>{value}</div>
      {sub&&<div style={{fontSize:12,color:(sub.startsWith("+")||sub.startsWith("▲"))?T.green:(sub.startsWith("-")||sub.startsWith("▼"))?T.red:T.txM}}>{sub}</div>}
      {trend&&trend.length>=2&&(
        <div style={{marginTop:2}}>
          <Sparkline data={trend} color={color} T={T}/>
          {trendLabel&&<div style={{fontSize:11,color:T.txD,marginTop:2,letterSpacing:0.3,textTransform:"uppercase",fontWeight:600}}>{trendLabel}</div>}
        </div>
      )}
      {tooltip&&hover&&(
