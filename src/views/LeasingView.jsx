  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
        ...
        <KpiCard icon={Banknote} label="Deuda pendiente c/IVA" value={fmtM(C.leasingDeudaTotalIVA||C.leasingDeudaTotal)} T={T} sub={`s/IVA: ${fmtM(C.leasingDeudaTotal)} · ${(C.leasingDeudaTotalUF||0).toLocaleString("es-CL",{maximumFractionDigits:0})} UF · al día de hoy`} color={T.red} colorBg={T.redBg}/>
      </div>
      ...
      <SectionCard title="Distribución de cuotas por día de pago" icon={Calendar} T={T} color={T.accent}>
        <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
          <div style={{flex:"1 1 150px",background:T.accentBg,borderRadius:10,padding:"12px 16px",border:`1px solid ${T.accent}22`}}>
            <div style={{fontSize:11,color:T.accent,fontWeight:600,marginBottom:4}}>DÍA 5</div>
          ...
          <div style={{flex:"1 1 150px",background:T.amberBg,borderRadius:10,padding:"12px 16px",border:`1px solid ${T.amber}22`}}>
            <div style={{fontSize:11,color:T.amber,fontWeight:600,marginBottom:4}}>DÍA 15</div>
          ...
          <div style={{flex:"1 1 150px",background:T.redBg,borderRadius:10,padding:"12px 16px",border:`1px solid ${T.red}22`}}>
            <div style={{fontSize:11,color:T.red,fontWeight:600,marginBottom:4}}>TOTAL MENSUAL</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(320px, 1fr))",gap:16}}>
        <SectionCard title="Cartera por emisor" icon={Building2} T={T} color={T.accent}><MiniTable T={T} headers={["Emisor","Contratos","Cuota c/IVA","Deuda c/IVA"]} rows={[...(C.leasingEmisores||[]).map(e=>[e.emisor,e.contratos,fmtM(e.cuotaIVA),fmtM(e.deudaIVA??e.deudaCLP)]),["TOTAL",C.leasingContratosActivos,fmtM(C.leasingTotalCuotaIVA),fmtM(C.leasingDeudaTotalIVA||C.leasingDeudaTotal)]]}/></SectionCard>
        <SectionCard title="Próximas cuotas a pagar" icon={Clock} T={T} color={T.amber}>
          {C.leasingProxCuotas?.length>0?(
            <MiniTable T={T} headers={["Fecha","Días","CLP c/IVA","Bancos","Estado"]} rows={C.leasingProxCuotas.map(r=>[r.fecha,r.dias,fmtM(r.cuotaIVA),r.bancos,<span key="e" style={{fontSize:11,fontWeight:600,padding:"2px 8px",borderRadius:4,background:r.estado==="URGENTE"?T.redBg:T.greenBg,color:r.estado==="URGENTE"?T.red:T.green}}>{r.estado}</span>])}/>
