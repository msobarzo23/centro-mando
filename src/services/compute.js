...
import { NO_FACTURADO_META, NO_FACTURADO_TOTAL, NO_FACTURADO_CLIENTES, NO_FACTURADO_MESES } from "../data/noFacturado.js";
...
export function computeAll(data) {
  ...
  const fondosRows=(data.finFondos||[]).filter(r=>r.Fondo||r.fondo);const fondosSaldos=fondosRows.filter(r=>parseNum(r["Valor Actual"]||r.ValorActual||r.valor_actual)>0).map(r=>({fondo:r.Fondo||r.fondo,admin:r.Administradora||r.administradora,invertido:parseNum(r["Monto Invertido"]||r.MontoInvertido||r.monto_invertido),actual:parseNum(r["Valor Actual"]||r.ValorActual||r.valor_actual),rentPct:r["Rentabilidad %"]||r.rentabilidad_pct||""}));const totalFondos=fondosSaldos.reduce((s,r)=>s+r.actual,0);
  // El DAP Crédito también es plata destinada a invertir (decisión de gerencia
  // 2026-07-13): entra en la inversión real junto al DAP Inversión y los FF.MM.
  const totalInversionReal=totalDAPInversion+totalDAPCredito+totalFondos;const totalInversiones=totalDAP+totalFondos;
  ...
  // son los viajes de ESTE mes (lag ~1 mes), así que se toma el valor medio real
  // por viaje valorizado (tarifa TMS + extras) y se escala a los viajes proyectados
  // del mes completo (norma por día de semana). Reemplaza al método viajes × tasa
  // histórica promedio cuando la cobertura del tarifario alcanza (≥70%); si viene
  // un reajuste MEPCO nuevo, la última tarifa queda corta — sesgo conservador.
  const proyMesSiguienteTarifario=(ingresoEstimado.cubiertos>0&&ingresoEstimado.cobertura>=0.7&&proyViajesHibrido>0)
    ?Math.round(ingresoEstimado.total/ingresoEstimado.cubiertos*proyViajesHibrido):0;

  // ══════════ VIAJES SIN FACTURAR (Consulta 77 del TMS) ══════════
  // Descarga semanal del TMS de viajes realizados con guía y nunca facturados
  // (meses cerrados, sin el mes anterior inmediato que aún puede facturarse).
  // Valorizados en scripts/generar_no_facturado.mjs; acá solo se publica.
  const noFacturado=(NO_FACTURADO_META&&NO_FACTURADO_META.viajes>0)
    ?{meta:NO_FACTURADO_META,total:NO_FACTURADO_TOTAL,clientes:NO_FACTURADO_CLIENTES,meses:NO_FACTURADO_MESES}:null;
